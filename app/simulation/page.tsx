...
const getChapterColor = (chapter: Chapter) => {
    ...
    return (
        defaultColors[chapter.chapterId] || {
            start: "from-gray-500",
            end: "to-gray-600",
        }
    );
    ...
    return (
        ...
        >
            ...
            <div className="relative bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-xl">
                ...
                <div
                    className={`absolute inset-0 bg-gradient-to-br ${colors.start} ${colors.end} opacity-0 group-hover:opacity-10 transition-opacity duration-500`}
                />

                <div className="p-6">
                    {/* Icon and chapter header */}

                    <div className="relative mb-4 flex items-start gap-3">
                        <div
                            className={`relative inline-flex p-3 rounded-xl bg-gradient-to-r ${colors.start} ${colors.end} text-white shadow-lg transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}
                        >
                            {getIcon(chapter.icon) || <Icons.Box className="w-8 h-8" />}
                        </div>
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400 mb-1">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-1 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <span>{chapter.simulationCount ?? 0} mô phỏng</span>
                        </div>
...
export default function SimulationPage() {
    ...
    useEffect(() => {
        Promise.all([fetch("/api/chapters"), fetch("/api/simulations")])

            .then(async ([chaptersRes, simulationsRes]) => {
                if (!chaptersRes.ok) throw new Error("Failed to fetch chapters");

                if (!simulationsRes.ok) throw new Error("Failed to fetch simulations");
                ...
                if (Array.isArray(chaptersData)) {
                    ...
                    chaptersArray = Object.values(chaptersData).filter(
                        (item): item is Chapter =>
                            !!item && typeof item === "object" && "chapterId" in item,
                    );
                }

                const simulationArray = Array.isArray(simulationsData.simulations)
                    ? simulationsData.simulations
                    : Array.isArray(simulationsData)
                        ? simulationsData
                        : [];

                const counts = simulationArray.reduce((acc: Record<string, number>, sim: any) => {
                    const id = sim.chapterId?.toString();
                ...
                setChapters(
                    chaptersArray.map((chapter) => ({
                        ...
                        simulationCount: counts[chapter.chapterId] ?? 0,
                    })),
                );
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredChapters.map((chapter) => (
                            <SimulationCard
                                ...
                                chapter={chapter}
                                onClick={() =>
                                    router.push(`/simulation/grade-11/${chapter.chapterId}`)
                                }
                            />
                        ))}
                    </div>
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div>
                            ...
                            <p className="text-purple-100 max-w-md">
                                Trải nghiệm dao động điều hòa với đồ họa 3D chân thực. Điều
                                chỉnh các thông số và quan sát sự thay đổi trong thời gian thực.
                            </p>
                        </div>
                        ...
                        {[
                            {
                                ...
                                description:
                                    "Hình ảnh 3D chân thực giúp hình dung rõ ràng các hiện tượng vật lý",
                            },

                            {
                                ...
                                description:
                                    "Xoay, zoom, điều chỉnh thông số và quan sát sự thay đổi trong thời gian thực",
                            },

                            {
                                ...
                                description:
                                    "Kết hợp lý thuyết và thực hành giúp ghi nhớ kiến thức một cách tự nhiên",
                            },
                        ].map((feature, index) => (
